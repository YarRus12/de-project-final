...
def get_execution_date(**context) -> str:
     "Функция извлекает execution_date"
...
def check_dwh(schema:str, vertica_connection:object):
...
def update_dwh(schema:str, table_name:str, vertica_connection:object):
     ...
     with vertica_connection:
          ...
          sql = f"""
                    ...
                    WITH cte AS ( -- выборка кода валюты, отношения валюты к доллару(430) и диапазона актуальности данных
                         SELECT  currency_code,
                              currency_code_with,
                              currency_with_div as div_to_usd,
                              date_update as effective_from,
                              COALESCE(LAG(date_update, 1) OVER (PARTITION BY currency_code ORDER BY date_update), '9999-12-31') as effective_to
                    ...
                    SELECT 
                         ...
                         SUM(amount*div_to_usd)/max(cnt_accounts_make_transactions) as avg_transactions_per_account, --средний объём транзакций с аккаунта
                         max(cnt_accounts_make_transactions) as cnt_accounts_make_transactions -- количество уникальных аккаунтов с совершёнными транзакциями по валюте
                    FROM
                         IAROSLAVRUSSUYANDEXRU__STAGING.transactions
                    INNER JOIN cte
                    ON transactions.currency_code = cte.currency_code
                         AND transaction_dt BETWEEN cte.effective_from and effective_to -- Объединяем по коду валюты и дате проведения транзакции и курса валюты
                    INNER JOIN table_accounts_transactions
                    ON transactions.currency_code = table_accounts_transactions.currency_code and transactions.transaction_dt::date = table_accounts_transactions.transaction_day
                    WHERE 1=1
                    AND account_number_from != '-1' -- убираем тестовые аккаутны
                    AND account_number_to != '-1' -- убираем тестовые аккаутны
                    AND transaction_dt::date = '{chosen_day}'::DATE - INTERVAL '1 DAY' -- выбираем транзакции за предыдущий день
                    GROUP BY transaction_dt::date, transactions.currency_code;"""
          cur.execute(sql)
          log.info(f"В таблицу {schema}.{table_name}: внесено {cur.fetchall()[0][0]} записей. Время завершения загрузки: {datetime.now().strftime('%Y-%m-%d %H:%M:%s')}")
...
check_dwh = PythonOperator(task_id='check_dwh',
                                 python_callable=check_dwh,
                                 op_kwargs={'schema': 'IAROSLAVRUSSUYANDEXRU__DWH', 'vertica_connection':VerticaHook().get_conn()},
                                 dag=dag)
update_dwh = PythonOperator(task_id='update_dwh',
                                 python_callable=update_dwh,
                                 op_kwargs={'schema': 'IAROSLAVRUSSUYANDEXRU__DWH', 'table_name':"global_metrics", 'vertica_connection':VerticaHook().get_conn()},
                                 dag=dag)
